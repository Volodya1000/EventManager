...
3. [📦 Архитектура и реализация проекта EventManager](#-архитектура-и-реализация-проекта-eventmanager)  
4. [🔐 Аутентификация и авторизация](#-аутентификация-и-авторизация)  
5. [✅ Валидация](#-валидация-данных)
6. [🧪 Тестирование](#-тестирование) 
7. [📊 База данных](#-база-данных)  
8. [⚠️ Безопасность](#-безопасность)  
9. [⚙️ Начальные настройки](#-начальные-настройки)  
...
# 📦 Архитектура и реализация проекта EventManager
...
- При успехе:
  - Генерируются:
    ...
    - Токен обновления (долгоживущий)
  - Токены сохраняются в HTTP-only, Secure, SameSite=Strict куки:
...
- **JWT Access Token:**
  ...
  - Срок жизни: `JwtOptions.ExpirationTimeInMinutes` (по умолчанию 15 минут)
- **Refresh Token:**
  - Криптографически безопасная случайная строка (64 байта)
  - Хранится в базе данных у пользователя
  - Срок жизни: 7 дней
...
- Реализована ролевая модель доступа (RBAC) через ASP.NET Identity
- Эндпоинты защищаются атрибутами: `[Authorize(Roles = "Admin")]`
- Системные роли:
  - `User` — базовые права
  - `Admin` — расширенные привилегии (назначение ролей и т.д.)
